...
struct MusicalTime {
    ...
    constexpr uint32_t count() const { return _ticks; }

    static constexpr MusicalTime beats(uint32_t n) { return MusicalTime(n * PPQN); }
    static constexpr MusicalTime ticks(uint32_t n) { return MusicalTime(n); }

    constexpr bool operator==(const MusicalTime& o) const { return _ticks == o._ticks; }
    constexpr bool operator!=(const MusicalTime& o) const { return _ticks != o._ticks; }
    constexpr bool operator< (const MusicalTime& o) const { return _ticks <  o._ticks; }
    constexpr bool operator>=(const MusicalTime& o) const { return _ticks >= o._ticks; }

    constexpr MusicalTime operator+(const MusicalTime& o) const { return MusicalTime(_ticks + o._ticks); }
    constexpr MusicalTime operator%(const MusicalTime& o) const { return MusicalTime(_ticks % o._ticks); }
    constexpr MusicalTime operator*(uint32_t scalar)      const { return MusicalTime(_ticks * scalar); }
